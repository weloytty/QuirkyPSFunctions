...
Task Clean -depends TestProperties -Description "Cleans out the destination module folder" {
  ...
  if (Test-Path $m_destinationPath -PathType Container)
  {
	...
	Write-Verbose "Removing $m_ModuleName"
	...
	Write-Verbose "Deleting $m_DestinationPath"
	Remove-Item $m_destinationPath -Recurse -Force
	Assert (-not (Test-Path $m_destinationPath -PathType Container)) -failureMessage "$m_DestinationPath not deleted"
